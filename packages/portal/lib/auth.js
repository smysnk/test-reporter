...
const DEVELOPMENT_AUTH_SECRET = 'test-station-portal-development-secret';
    ...
    callbacks: {
      async jwt({ token, user }) {
        if (user) {
          ...
          token.userId = token.userId || token.sub || token.email || 'portal-user';
        }
...
export async function getPortalSession(req, res, options = {}) {
  return getServerSession(req, res, createAuthOptions(options));
}

export async function requirePortalSession(context, options = {}) {
  const session = await getPortalSession(context.req, context.res, options);
  if (session) {
    return {
      session,
      redirect: null,
...
export function buildPortalActorHeaders(session) {
  if (!session) {
    return {};
  }

  return {
    'x-test-station-actor-id': session.userId || session.user?.email || 'portal-user',
    'x-test-station-actor-email': session.user?.email || '',
    'x-test-station-actor-name': session.user?.name || session.user?.email || session.userId || 'Portal User',
...
export function resolveDefaultProjectKeys(options = {}) {
  const values = normalizeProjectKeys(
    ...
    splitConfiguredValues(env.get('PORTAL_DEFAULT_PROJECT_KEYS').default('*').asString()),
  );
...
export function resolveAdminEmails(options = {}) {
  ...
  return normalizeEmailList(splitConfiguredValues(env.get('PORTAL_ADMIN_EMAILS').default('').asString()));
}
