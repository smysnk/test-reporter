...
export function CoverageTrendPanel({ title, subtitle = null, points = [], overlays = [], compact = false }) {
  ...
  return React.createElement(
    'article',
    { className: compact ? 'portal-trend-card portal-trend-card--compact' : 'portal-trend-card' },
    React.createElement(
      'div',
      ...
      React.createElement(
        ...
        React.createElement('strong', { className: 'portal-list__title' }, title),
        subtitle ? React.createElement('span', { className: 'portal-list__meta' }, subtitle) : null,
      ),
      React.createElement(
        'div',
        { className: 'portal-stack' },
        React.createElement('strong', { className: 'portal-trend-card__value' }, formatCoveragePct(latestPoint?.linesPct)),
        React.createElement('span', { className: 'portal-list__meta' }, formatSignedDelta(delta)),
      ),
    ),
    normalizedPoints.length > 0
      ? React.createElement(
        ...
        React.createElement(
          'svg',
          { className: 'portal-trend-card__chart', viewBox: '0 0 320 120', preserveAspectRatio: 'none', role: 'img', 'aria-label': `${title} trend` },
          React.createElement('path', {
            className: 'portal-trend-card__baseline',
            d: 'M 0 108 L 320 108',
          }),
          React.createElement('polyline', {
            className: 'portal-trend-card__line',
            points: chartPoints.map((point) => `${point.x},${point.y}`).join(' '),
          }),
          ...chartPoints.map((point) => React.createElement('circle', {
            key: point.key,
            className: 'portal-trend-card__dot',
            cx: point.x,
            cy: point.y,
            r: point.latest ? 4 : 3,
          })),
          ...createOverlayMarkers(normalizedPoints, overlays).map((overlay) => React.createElement('g', {
            key: overlay.key,
            className: 'portal-trend-card__overlay',
          },
        ...
        React.createElement(
          'div',
          { className: 'portal-list__row' },
          React.createElement('span', { className: 'portal-list__meta' }, formatDateTime(latestPoint?.completedAt || latestPoint?.recordedAt)),
          React.createElement('span', { className: 'portal-list__meta' }, `${normalizedPoints.length} points`),
        ),
        overlays.length > 0
          ? React.createElement(
            'div',
            ...
            ...overlays.slice(0, 4).map((overlay) => React.createElement(
              'span',
              { className: `portal-chip ${overlay.kind === 'release' ? 'portal-chip--release' : ''}`, key: `${overlay.kind}:${overlay.label}:${overlay.recordedAt}` },
              overlay.kind === 'release' ? `release ${overlay.label}` : `version ${overlay.label}`,
            )),
          )
          : null,
      )
      : React.createElement('span', { className: 'portal-list__meta' }, 'No historical points available.'),
  );
}
