...
import { loadProjectBenchmarkNamespace, loadProjectBenchmarkOverview, loadProjectExplorerPage } from '../../../../lib/serverGraphql.js';
...
export default async function handler(req, res) {
  ...
  const statGroup = typeof req.query?.statGroup === 'string' ? req.query.statGroup.trim() : '';
  if (!slug) return;
  try {
    ...
    const data = statGroup
      ? await loadProjectBenchmarkNamespace({ session, projectKey: shell.project.key, statGroup, requestTrace: trace })
      : await loadProjectBenchmarkOverview({ session, projectKey: shell.project.key, requestTrace: trace });
