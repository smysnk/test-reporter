...
import { buildPortalActorHeaders, createAuthOptions } from '../packages/portal/lib/auth.js';
import { formatCoveragePct, formatDuration } from '../packages/portal/lib/format.js';
import {
  executePortalGraphql,
  loadPortalHomePage,
  loadProjectExplorerPage,
  loadRunExplorerPage,
} from '../packages/portal/lib/serverGraphql.js';
import { buildSignInRedirectUrl, isProtectedPortalPath } from '../packages/portal/lib/routeProtection.js';

test('portal auth options expose the sign-in page and session actor metadata', async () => {
...
test('portal actor headers and route protection helpers produce the expected auth wiring', () => {
  const headers = buildPortalActorHeaders({
    userId: 'user-1',
    user: {
  ...
  assert.deepEqual(headers, {
    ...
    'x-test-station-actor-name': 'Portal User',
    'x-test-station-actor-role': 'member',
    'x-test-station-actor-project-keys': 'workspace,api',
  });

  assert.equal(isProtectedPortalPath('/'), true);
  assert.equal(isProtectedPortalPath('/projects/workspace'), true);
  assert.equal(isProtectedPortalPath('/runs/run-1'), true);
  assert.equal(isProtectedPortalPath('/auth/signin'), false);
  assert.equal(buildSignInRedirectUrl('/runs/run-1'), '/auth/signin?callbackUrl=%2Fruns%2Frun-1');
});

test('portal GraphQL helpers forward actor headers and combine project activity data', async () => {
  const session = {
    userId: 'user-1',
    user: {
      email: 'user@example.com',
  ...
  const fetchImpl = async (_url, options) => {
    ...
    if (query.includes('PortalHomePage')) {
      return new Response(JSON.stringify({
        data: {
          me: { id: 'user-1', name: 'Portal User', email: 'user@example.com', role: 'member', projectKeys: ['workspace'] },
    ...
    if (query.includes('PortalProjectBySlug')) {
      return new Response(JSON.stringify({
        data: {
          project: {
            id: 'project-1',
    ...
    if (query.includes('PortalProjectActivity')) {
      return new Response(JSON.stringify({
        data: {
          runs: [{
            id: 'run-1',
    ...
    if (query.includes('PortalRunScopeTrendCatalog')) {
      return new Response(JSON.stringify({
        data: {
    ...
    throw new Error(`Unexpected portal GraphQL request: ${query}`);
  };

  const home = await loadPortalHomePage({ session, fetchImpl, requestId: 'req-home' });
...
test('portal run loader and raw GraphQL executor preserve response structure', async () => {
  const session = {
    userId: 'user-1',
    user: {
      email: 'user@example.com',
  ...
  const direct = await executePortalGraphql({
    session,
