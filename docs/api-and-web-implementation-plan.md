# Test Station API and Web Implementation Plan

## Status

- Proposed.
- Completed now:
  - Phase 1 workspace scaffold for `packages/server` and `packages/web`
  - shared env bootstrap at `config/env.mjs`
  - server health and GraphQL scaffold
  - web provider shell and route scaffold
  ...
  - Phase 5 NextAuth web shell, protected routes, same-origin GraphQL proxy, and SSR explorer pages
  - Phase 6 precomputed coverage trend storage, run-to-run comparison queries, and historical web trend views
- Scope in this document:
  - add one long-running Node server package for ingestion, storage, and GraphQL
  - add one Next.js web package for exploration, auth, and historical reporting
...
3. Add a web that can browse projects, runs, artifacts, failures, and coverage trends over time.
...
- GraphQL API for web and internal automation
...
- SSO-authenticated operator/admin web
...
- Add the new server and web as non-publishable application workspaces.
...
This keeps the ingest path simple while preserving proper user identity and auditability in the web.
...
- import that helper from both `packages/server` and `packages/web`
...
- GraphQL reads for web require authenticated user context
...
- use the varcad-style provider composition:
  ...
  - global styles
  - route-level shell
- establish one web design system early
...
- add root scripts for server and web dev/build/lint
...
- use Next.js Pages Router in web to stay aligned with the varcad auth/provider patterns

### Acceptance criteria

- `yarn workspace server dev` starts a health endpoint
- `yarn workspace web dev` starts a Next.js shell
...
- web can fetch all data from GraphQL without REST fallbacks
- GraphQL unauthorized requests fail consistently
- same-origin `/graphql` is usable from the web app

## Phase 5: Web Shell, SSO, and Explorer

### Deliverables

- NextAuth setup copied from varcad patterns
- route middleware for protected web routes
...
- web shell uses one consistent theme/provider stack
...
- web can show line coverage changes over time
...
- SSO-authenticated same-origin GraphQL for web
...
### Alternative 3: no Redux in web

Why only partially recommended:

- viable for small webs
...
- custom report-schema mutation by web users
...
3. Add read-only GraphQL and a read-only web explorer before any write-heavy admin tooling.
...
- web users can browse projects and inspect individual test executions
