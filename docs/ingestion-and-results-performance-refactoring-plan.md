...
- Implementation is in progress in the local worktree. A 2026-08-21 code/HAR audit confirmed that the deployed application still uses the pre-refactor read path and identified correctness and scale gaps that must be resolved before the local implementation is treated as deployable.
...
- Phase 5: implemented and browser-validated. Suites load 100 tests at a time with cancellation, server filters, stable BFF errors, independent panel retry state, and windowed rendering. The 10,000-test controlled run rendered 13 test rows; five measured executions produced p95 values of `71.5 ms` for suite expansion and `191 ms` for the second page.
- Phase 6: implementation is ready for rollout. Read and ingest roles are isolated, the read service is cluster-internal, migrations/backfill run as dedicated locked jobs, reads/web are replicated with readiness and disruption budgets, runtime metrics/dashboards/alerts/runbooks exist, the release is no longer blocked by npm publishing, and the required post-deploy benchmark publishes evidence before enforcing budgets. Exact-image deployment, shadow parity observation, rollback exercise, and three consecutive production-green checkpoints remain evidence gates and cannot be claimed before the rollout runs.
